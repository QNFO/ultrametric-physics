---
title: 'Valuation Without $\mathbb{R}$: A Category-Theoretic Foundation for Finite Measurement'
author: "QNFO"
date: "2026-08-04"
...
**Dependency 1: The real numbers ($\mathbb{R}$).** The Archimedean completion of $\mathbb{Q}$ is ONE
completion among ALL completions — per Ostrowski's theorem, the valuations of $\mathbb{Q}$
...
$\mathbb{R}$ without justification is the **Archimedean bias** — an unexamined default
...
10.5281/zenodo.21672990]`. Only the computable Archimedean continuum $\mathbb{R}_c$ is
physical; the uncountable breadth of $\mathbb{R}$ is eliminated.
...
for some base $q > 1$. When $q$ is prime, $d_v$ is precisely the p-adic metric on $\mathbb{Q}_p$.
The valuation space $(\mathcal{S}, v)$ is equivalent to the ultrametric space $(\mathcal{S}, d_v)$ — the
...
$$
with restriction maps $\pi_{r+1}: \mathcal{S}_{r+1} \to \mathcal{S}_r$ is a presheaf on $\mathbb{N}^{op}$.

**Sheaf condition:** For any cover of resolution r by finer resolutions
{r+1, r+2, ...}, the states in \mathcal{S}_r are determined by their refinements:
...
At resolution r, the measurement partitions the state space into $\lvert\mathcal{S}_r\rvert$ equivalence
classes. The information content of the measurement is:

$$
$I(r) = \log_2 \lvert\mathcal{S}_r\rvert \ \text{bits}$
$$
...
The valuation space $(\mathbb{Q}, v_p)$ satisfies V1–V3 exactly. The p-adic numbers $\mathbb{Q}_p$ are
the Cauchy completion of $\mathbb{Q}$ under the metric $d_p(x, y) = p^{-v_p(x, y)}$. The
distinguishability graph $G_r$ of $(\mathbb{Q}, v_p)$ is the projection of the Bruhat–Tits
tree at depth r.
...
PGL(2, $\mathbb{Q}_p$). Its vertices at depth r classify p-adic disks of radius $p^{-r}$
The boundary at infinity is ℙ^1($\mathbb{Q}_p$).
...
The refinement maps $\pi_{r+1}: \mathcal{S}_{r+1} \to \mathcal{S}_r$ form a sheaf over $\mathbb{N}^{op}$. The consistency
...
$$
where ℱ_ref is the refinement sheaf over $\mathbb{N}$^op. For d = 3 (spatial dimensions),
...
**Δlog-odds:** $P(\text{ultrametric clustering} \mid \text{random graph}) \ll 1$ for large $\lvert\mathcal{S}\rvert$.
Random metrics are NOT ultrametric with high probability. Genuine risky
prediction — $P(O \mid \lnot T) \approx 0$ `[EVIDENCE — pre-registered, falsifiable]`
...
**Δlog-odds:** $\approx 0$ (both exponential and power-law are expected GROWTH forms;
